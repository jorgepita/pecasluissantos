...
> The storefront header/footer render live `store_settings`. The site is
> **live on GitHub Pages** (see "Deployment" below), with per-page SEO
> metadata, `robots.txt`, and a CI-generated `sitemap.xml`. Payments,
> orders, customer accounts and everything else business-specific beyond
> browsing/contact is **not implemented yet** — see
...
to `main` lints, format-checks, builds, generates `sitemap.xml`
(`scripts/generate-sitemap.mjs`), and publishes `dist/` to GitHub Pages.
See [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) ("Deployment strategy",
"SEO / metadata") for the full mechanics (base path, SPA fallback,
sitemap generation, etc.).

Required one-time repository configuration (already done for this repo —
noted here for reference / for pointing this project at a different
Supabase project or repo):

1. **Settings → Pages → Build and deployment → Source**: **"GitHub
   Actions"** (not the legacy branch-based option).
...
URL/Redirect URLs allowlist. Still, it's worth adding the production URL
above under Supabase Dashboard → Authentication → URL Configuration →
Site URL (and Redirect URLs) so the project is correctly configured
ahead of any future redirect-based auth flow (e.g. password reset).
